...
const AddAppointment = () => {
  ...
  return (
    <div>
    <button onClick={() => { setToggleForm(prevToggleFrom => !prevToggleFrom)}} 
      className={` bg-blue-400 text-white px-2 py-3 w-full text-left ${toggleForm ? "rounded-t-md" : "rounded-md"}`}>
      <div><BiCalendarPlus className="text-xl inline-block align-text-top" />  Add Appointment</div>
    </button>
    {
      toggleForm && 
      <div className="transition 
      ease-out duration-500 border-r-2 border-b-2 border-l-2 border-light-blue-500 rounded-b-md pl-4 pr-4 pb-4">
